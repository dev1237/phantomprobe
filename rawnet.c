...
struct cap {
    ...
    rst_ev *rst; int rn; icmp_ev *icmp; int in;
};
...
cap_t *cap_start(const char *dev, uint32_t target_be) {
    ...
    if (!c) return NULL;
    c->rst = malloc(CAPMAX * sizeof(rst_ev)); c->icmp = malloc(CAPMAX * sizeof(icmp_ev));
    c->target = target_be; pthread_mutex_init(&c->mu, NULL);
    char err[PCAP_ERRBUF_SIZE];
    c->ph = pcap_create(dev, err);
    if (!c->ph) { free(c->rst); free(c->icmp); free(c); return NULL; }
    pcap_set_snaplen(c->ph, 128);
    pcap_set_promisc(c->ph, 0);
    pcap_set_timeout(c->ph, 40);
    pcap_set_immediate_mode(c->ph, 1);
    if (pcap_activate(c->ph) != 0) { pcap_close(c->ph); free(c->rst); free(c->icmp); free(c); return NULL; }
    ...
    snprintf(filt, sizeof filt, "icmp or (tcp and src host %s)", ipbuf);
...
void cap_reset(cap_t *c) { pthread_mutex_lock(&c->mu); c->rn = 0; c->in = 0; pthread_mutex_unlock(&c->mu); }
...
void cap_stop(cap_t *c) {
    ...
    pcap_close(c->ph); free(c->rst); free(c->icmp);
